...
class Seq2seq(chainer.Chain):
    ...
    def CalcLoss(self, xs, ys):
        ...
        h1, hf = self.encoder_f(None, exs_f)
        h2, hb = self.encoder_b(None, exs_b)
        # 隠れ状態ベクトルの集合
...
def main():
    global target_words, target_word_ids, target_chars, target_char_ids,source_word_ids,source_char_ids, start
    ...
    print('max_source_word_id:', np.max(list(source_word_ids.values())))
    print('max_source_char_id:', np.max(list(source_char_ids.values())))
    ...
    #train_source, test_source = load_data(args.SOURCE)
    #train_target, test_target = load_data(args.TARGET)
    ...
    model = Seq2seq(args.layer, len(source_word_ids), len(target_word_ids), len(source_char_ids), len(target_char_ids), args.unit, len(train_data))
    if args.gpu >= 0:
